...
def fetch_data():
    try:
        ...
        # Check for failed status (assuming "failed" or "rejected" values)
        if "Status" in df.columns:
...
def get_custom_date_range():
    ...
    today = datetime.now()
    if today.day >= 10:
        # Current period: from 10th of this month to 10th of next month
        ...
        # Current period: from 10th of previous month to 10th of this month
        if today.month == 1:
...
def ranges():
    ...
    # Custom date range (10th to 10th)
...
def view_failed_transfers(k):
    ...
    with c1:
        ...
        if not failed_by_agent.empty:
            # Create bar chart for failed transfers
...
def view_agent_success_rates(k):
    ...
    if agent_rates:
        ...
        # Create dual-axis chart for success vs failure rates
...
def main():
    ...
    with st.sidebar:
        ...
        if not st.session_state.authenticated:
            ...
            st.success(f"Logged in as: {st.session_state.view_mode.title()}")
            if st.button("Logout"):
                st.session_state.authenticated = False
                st.session_state.view_mode = "user"
                st.rerun()
    ...
    if not st.session_state.authenticated:
        ...
        <div class="login-container">
            <h2>⚡ Sales Transfer Dashboard</h2>
            <p style="margin: 20px 0;">Please login as Admin or continue as User</p>
        </div>
        """, unsafe_allow_html=True)
        return
