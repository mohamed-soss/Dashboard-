...
.admin-header h1 { color: white !important; font-size: 30px; font-weight: 900; margin: 0; letter-spacing: -1px; position: relative; z-index: 2; }
.admin-header p { color: rgba(255,255,255,0.9) !important; font-size: 14px; margin: 6px 0 0 0; position: relative; z-index: 2; }
.admin-badge {
    ...
    font-size: 12px; color: white !important; font-weight: 700;
...
.main-header h1 { color: white !important; font-size: 30px; font-weight: 900; margin: 0; letter-spacing: -1px; position: relative; z-index: 2; }
.main-header p { color: rgba(255,255,255,0.9) !important; font-size: 14px; margin: 6px 0 0 0; position: relative; z-index: 2; }
.header-badges { display: flex; gap: 10px; margin-top: 14px; position: relative; z-index: 2; flex-wrap: wrap; }
.h-badge {
    display: inline-flex; align-items: center; gap: 6px;
...
.h-badge:hover { background: rgba(255,255,255,0.15); transform: translateY(-1px); }
.live-dot {
...
div[data-testid="stMetric"] label { color: white !important; font-size: 11px !important; font-weight: 700 !important; text-transform: uppercase !important; letter-spacing: 1px !important; }
div[data-testid="stMetric"] div[data-testid="stMetricValue"] { color: white !important; font-size: 36px !important; font-weight: 900 !important; letter-spacing: -1.5px !important; animation: countPop 0.7s cubic-bezier(0.16,1,0.3,1) 0.5s both; }
div[data-testid="stMetric"] div[data-testid="stMetricDelta"] { color: rgba(255,255,255,0.85) !important; font-size: 12px !important; font-weight: 500 !important; }

/* ==================== SECTION HEADERS ==================== */
.section-title {
    color: white; font-size: 20px; font-weight: 800;
    margin: 36px 0 18px 0; padding-bottom: 12px;
...
.section-dot-red { background: #EF4444; box-shadow: 0 0 12px rgba(239,68,68,0.4); }
...
.glass-card h3, .glass-card strong { color: white !important; }
.glass-card p, .glass-card span { color: rgba(255,255,255,0.85) !important; }
.card-ico {
...
.glass-card:hover .card-ico { transform: rotate(-5deg) scale(1.1); box-shadow: 0 12px 28px rgba(79,70,229,0.4); }
.card-ico-red { background: linear-gradient(135deg, #EF4444, #DC2626); box-shadow: 0 8px 20px rgba(239,68,68,0.3); }
.glass-card:hover .card-ico-red { box-shadow: 0 12px 28px rgba(239,68,68,0.4); }
...
.s-done { background: linear-gradient(135deg, rgba(16,185,129,0.1), rgba(16,185,129,0.05)); border: 1px solid rgba(16,185,129,0.2); }
.s-pending { background: linear-gradient(135deg, rgba(245,158,11,0.1), rgba(245,158,11,0.05)); border: 1px solid rgba(245,158,11,0.2); }
.s-rate { background: linear-gradient(135deg, rgba(79,70,229,0.1), rgba(79,70,229,0.05)); border: 1px solid rgba(79,70,229,0.2); }
.s-failed { background: linear-gradient(135deg, rgba(239,68,68,0.1), rgba(239,68,68,0.05)); border: 1px solid rgba(239,68,68,0.2); }
.s-ico { font-size: 30px; margin-bottom: 8px; }
.s-val { font-size: 34px; font-weight: 900; letter-spacing: -1px; animation: countPop 0.7s cubic-bezier(0.16,1,0.3,1) 0.4s both; }
...
.top-hero:hover { transform: translateY(-3px); box-shadow: 0 12px 32px rgba(245,158,11,0.12); }
...
.rank-low { background: rgba(239,68,68,0.04) !important; border-color: rgba(239,68,68,0.1) !important; }
.rank-low:hover { border-color: rgba(239,68,68,0.25) !important; box-shadow: 0 8px 24px rgba(239,68,68,0.08) !important; }
.bar-track { background: rgba(255,255,255,0.06); border-radius: 4px; height: 5px; margin-top: 6px; overflow: hidden; }
.bar-fill { height: 100%; border-radius: 4px; background: linear-gradient(90deg, var(--primary), var(--accent)); animation: barGrow 1.2s cubic-bezier(0.16,1,0.3,1) forwards; }
...
.highlight-box:hover { transform: translateY(-3px); box-shadow: 0 12px 32px rgba(79,70,229,0.12); border-color: rgba(129,140,248,0.3); }
.hl-ico {
...
.insight-danger { background: rgba(239,68,68,0.06); border: 1px solid rgba(239,68,68,0.12); }
.insight-success { background: rgba(16,185,129,0.06); border: 1px solid rgba(16,185,129,0.12); }
...
.failed-card:hover { transform: translateY(-4px); box-shadow: 0 16px 40px rgba(239,68,68,0.12); border-color: rgba(239,68,68,0.3); }
.failed-detail-row {
    display: flex; align-items: center; justify-content: space-between;
...
.failed-detail-row:hover { background: rgba(239,68,68,0.08); border-color: rgba(239,68,68,0.2); transform: translateX(4px); }
.failed-bar-track { background: rgba(235,68,68,0.1); border-radius: 4px; height: 5px; margin-top: 5px; overflow: hidden; }
.failed-bar-fill { height: 100%; border-radius: 4px; background: linear-gradient(90deg, #EF4444, #F59E0B); animation: barGrow 1.2s cubic-bezier(0.16,1,0.3,1) forwards; }

/* ==================== TABS ==================== */
.stTabs [data-baseweb="tab-list"] { gap: 4px; background: rgba(255,255,255,0.04); border-radius: 12px; padding: 4px; border: 1px solid var(--border); }
.stTabs [data-baseweb="tab"] { background: transparent; border-radius: 10px; padding: 8px 20px; border: none !important; color: white !important; font-weight: 600; font-size: 13px; transition: all 0.3s ease; }
.stTabs [aria-selected="true"] { background: rgba(79,70,229,0.2) !important; color: white !important; font-weight: 700; box-shadow: 0 0 20px rgba(79,70,229,0.15); }
.stTabs [data-baseweb="tab"]:hover:not([aria-selected="true"]) { color: white !important; background: rgba(255,255,255,0.04); }
...
.stDataFrame th { background: rgba(79,70,229,0.1) !important; color: white !important; font-weight: 700 !important; }
.stDataFrame td { color: white !important; }

/* ==================== SELECTBOX ==================== */
.stSelectbox label { color: white !important; }
.stSelectbox > div > div { border-radius: 10px !important; border-color: var(--border2) !important; background: rgba(255,255,255,0.04) !important; color: white !important; }
...
.admin-banner { background: linear-gradient(135deg, rgba(239,68,68,0.08), rgba(239,68,68,0.03)); border: 1px solid rgba(239,68,68,0.15); }
...
p, span, label, div, td, th, li, a, h1, h2, h3, h4, h5, h6, strong, em, b, i, small { color: white !important; }
</style>
...
(function() {
    ...
    const NUM = 80, MAX_DIST = 150, dots = [];
    ...
    function draw() {
        ...
        for (let i = 0; i < NUM; i++) {
            for (let j = i + 1; j < NUM; j++) {
                ...
                if (dist < MAX_DIST) {
                    ctx.beginPath();
                    ctx.strokeStyle = `rgba(129,140,248,${(1 - dist / MAX_DIST) * 0.25})`;
                    ctx.lineWidth = 0.8;
...
# ============================================================
# HELPERS — MONTH RANGE FIXED: 10th → 10th
# ============================================================
def ranges():
    n = datetime.now()
    ...
    # --- Month: 10th of current month → 10th of next month ---
    if n.day >= 10:
        # Current period started on the 10th of this month
        ms = datetime(n.year, n.month, 10)
        if n.month == 12:
            me = datetime(n.year + 1, 1, 10)
        else:
            me = datetime(n.year, n.month + 1, 10)
        # Previous period: 10th of last month → 10th of this month
        if n.month == 1:
            lms = datetime(n.year - 1, 12, 10)
        else:
            lms = datetime(n.year, n.month - 1, 10)
        lme = ms
    else:
        # Before the 10th → current period started on 10th of LAST month
        if n.month == 1:
            ms = datetime(n.year - 1, 12, 10)
        else:
            ms = datetime(n.year, n.month - 1, 10)
        me = datetime(n.year, n.month, 10)
        # Previous period: 10th of two months ago → 10th of last month
        if n.month <= 2:
            if n.month == 2:
                lms = datetime(n.year - 1, 12, 10)
            else:
                lms = datetime(n.year - 1, 11, 10)
        else:
            lms = datetime(n.year, n.month - 2, 10)
        lme = ms
...
def calc(df, multiplier=1):
    ...
    return {
        "total": total,
        "done": dn * m, "done_raw": dn,
        "pend": pend,
        "failed": fl * m, "failed_raw": fl,
        "rate": (dn / total * 100) if total > 0 else 0,
        "dest": tc.index[0] if not tc.empty else "N/A",
        "dest_n": int(tc.iloc[0]) * m if not tc.empty else 0,
        "tc": tc * m if not tc.empty else tc, "tc_raw": tc,
        "ac": ac * m if not ac.empty else ac, "ac_raw": ac,
        "fc": fc * m if not fc.empty else fc, "fc_raw": fc,
        "ac_t": td["Agent Name"].value_counts() * m if "Agent Name" in td.columns and not td.empty else pd.Series(),
        "ac_t_raw": td["Agent Name"].value_counts() if "Agent Name" in td.columns else pd.Series(),
        "ac_w": tw["Agent Name"].value_counts() * m if "Agent Name" in tw.columns and not tw.empty else pd.Series(),
        "ac_w_raw": tw["Agent Name"].value_counts() if "Agent Name" in tw.columns else pd.Series(),
        "ac_m": tm["Agent Name"].value_counts() * m if "Agent Name" in tm.columns and not tm.empty else pd.Series(),
        "ac_m_raw": tm["Agent Name"].value_counts() if "Agent Name" in tm.columns else pd.Series(),
        "fc_t": fc_t * m if not fc_t.empty else fc_t, "fc_t_raw": fc_t,
        "fc_w": fc_w * m if not fc_w.empty else fc_w, "fc_w_raw": fc_w,
        "fc_m": fc_m * m if not fc_m.empty else fc_m, "fc_m_raw": fc_m,
        "low": ac.idxmin() if len(ac) > 1 else "N/A",
        "td": len(td) * m, "yd": len(yd) * m, "dp": pc(len(td), len(yd)),
        "tw": len(tw) * m, "lw": len(lw) * m, "wp": pc(len(tw), len(lw)),
        "tm": len(tm) * m, "lm": len(lm) * m, "mp": pc(len(tm), len(lm)),
        "fl_t": len(fl_t) * m, "fl_w": len(fl_w) * m, "fl_m": len(fl_m) * m,
...
def view_login():
    ...
    if st.session_state.login_attempt:
        st.markdown('<div class="login-error"><span style="color:#FCA5A5 !important;">Invalid password. Please try again.</span></div>', unsafe_allow_html=True)

    password = st.text_input("Password", type="password", key="admin_pw", placeholder="Enter admin password...")
...
def view_header(is_admin=False):
    if is_admin:
        ...
        <div class="admin-header">
            ...
            <p>Admin view — sales counted at 10x · Failed transfers visible · Month: 10th → 10th</p>
        ...
        <div class="main-header">
            ...
            <p>Real-time monitoring — only completed (done) transfers are counted · Month: 10th → 10th</p>
            <div class="header-badges">
                <div class="h-badge"><div class="live-dot"></div> Live</div>
                <div class="h-badge">🔄 Auto-refresh 45s</div>
                <div class="h-badge">📅 {datetime.now().strftime("%b %d, %Y %H:%M")}</div>
    ...
    <div class="glass-card">
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">
            ...
            <div>
                ...
                <p style="margin:2px 0 0 0;font-size:12px;color:rgba(255,255,255,0.85) !important;">{"Admin: counts at 10x · " if is_admin else ""}Month period: 10th → 10th</p>
            </div>
        </div>
...
def view_failed_transfers(k):
    ...
    if fc.empty:
        ...
        <div class="glass-card" style="text-align:center;padding:40px;">
            ...
            <p style="color:rgba(255,255,255,0.6) !important;font-size:13px;margin-top:6px;">All transfers completed successfully</p>
        </div>""", unsafe_allow_html=True)
        return
    ...
    <div class="glass-card">
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">
            ...
            <div>
                ...
                <p style="margin:2px 0 0 0;font-size:12px;color:rgba(255,255,255,0.85) !important;">Raw count per agent · Month: 10th → 10th</p>
            </div>
        </div>
    """, unsafe_allow_html=True)
                ...
                for i, (agent, cnt) in enumerate(raw_data.items(), 1):
                    ...
                    <div class="failed-detail-row" style="animation-delay:{i*0.07}s;">
                        <div style="flex:1;">
                            ...
                            <div class="failed-bar-track"><div class="failed-bar-fill" style="width:{bw}%;animation-delay:{i*0.07}s;"></div></div>
                        </div>
    ...
    if not ac.empty and not fc.empty:
        ...
        comp_data = [{"Agent": a, "Completed": int(ac.get(a, 0)), "Failed": int(fc.get(a, 0))} for a in all_agents]
        comp_melted = pd.DataFrame(comp_data).melt(id_vars="Agent", var_name="Type", value_name="Count")
        fig = px.bar(comp_melted, x="Agent", y="Count", color="Type", barmode="group",
        ...
        rate_data = []
        for a in all_agents:
            c_val = int(ac.get(a, 0))
            f_val = int(fc.get(a, 0))
            tot = c_val + f_val
            rate = (c_val / tot * 100) if tot > 0 else 0
            rate_data.append({"Agent": a, "Success Rate %": round(rate, 1), "Total": tot})
        ...
        fig2 = px.bar(rate_df, y="Agent", x="Success Rate %", orientation="h",
                      color="Success Rate %", color_continuous_scale=["#EF4444", "#F59E0B", "#10B981"],
                      range_color=[0, 100], title="Success Rate per Agent (%)")
                ...
                if not counts.empty:
                    ...
                    <div class="top-hero">
                        ...
                        <div style="display:flex;align-items:baseline;gap:6px;">
                            ...
                            <span style="font-size:13px;color:rgba(255,255,255,0.85) !important;">completed{" (raw: " + str(raw_cnt) + ")" if is_admin else ""}</span>
                        </div>
                    </div>""", unsafe_allow_html=True)
                else:
    ...
    with c2:
        ...
        if not ac.empty:
            ...
            for i, (ag, cnt) in enumerate(ac.head(10).items(), 1):
                ...
                raw_sfx = f' <span style="font-size:10px;color:rgba(255,255,255,0.4) !important;">(raw:{raw_val})</span>' if is_admin else ""
                st.markdown(f"""
                <div class="rank-row {"rank-low" if is_low else ""}" style="animation-delay:{i*0.06}s;">
                    ...
                    <span style="color:white !important;font-weight:800;font-size:18px;margin-left:12px;">{int(cnt)}{raw_sfx}</span>
                </div>""", unsafe_allow_html=True)
            if len(ac) > 1:
                st.markdown(f"""
...
def view_transfers(k, is_admin=False):
    ...
    if not tc.empty:
        ...
        with c1:
            fig = px.pie(values=plot_tc.values, names=plot_tc.index,
                         title=f"Distribution{' (Raw)' if is_admin else ''}",
            ...
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            tf = plot_tc.reset_index(); tf.columns = ['Destination', 'Count']
            tf = tf.sort_values('Count', ascending=True)
            fig = px.bar(tf, y='Destination', x='Count', title=f"By Destination{' (Raw)' if is_admin else ''}",
                         orientation='h', color='Count', color_continuous_scale=['rgba(255,255,255,0.05)', '#4F46E5'])
        ...
        <div class="highlight-box">
            ...
            <div>
                ...
                <div style="font-size:12px;color:rgba(255,255,255,0.85) !important;margin-top:2px;">{k['dest_n']} transfers{" · raw: " + str(dest_raw) if is_admin else ""}</div>
            </div>
        </div>""", unsafe_allow_html=True)
    else:
...
def view_agents(k, is_admin=False):
    ...
    if not ac.empty:
        ...
        if sel == "All Agents":
            filt = dd; filt_fail = fd
            t, w, m_ = k["td"], k["tw"], k["tm"]
            ...
            t, w, m_ = t_raw * mult, w_raw * mult, m_raw * mult

        done_val = len(filt) * mult
        fail_val = len(filt_fail) * mult
        ...
        with mc4: st.metric("🗓️ This Month", f"{m_}", f"raw: {m_raw}" if is_admin else None)

        if is_admin:
            mc5, mc6 = st.columns(2)
            with mc5: st.metric("❌ Total Failed", f"{fail_val}", f"raw: {len(filt_fail)}")
            with mc6:
                fr = (len(filt_fail) / (len(filt) + len(filt_fail)) * 100) if (len(filt) + len(filt_fail)) > 0 else 0
                st.metric("⚠️ Failure Rate", f"{fr:.1f}%", f"{len(filt_fail)} of {len(filt) + len(filt_fail)}")
        ...
        if not filt.empty:
            ...
            if avail:
                show = filt[avail].sort_values("Timestamp", ascending=False).head(10).copy()
        ...
        if is_admin and not filt_fail.empty:
            ...
            if avail_f:
                show_f = filt_fail[avail_f].sort_values("Timestamp", ascending=False).head(10).copy()
...
def view_trends(k, is_admin=False):
    ...
    st.markdown('<div class="glass-card">', unsafe_allow_html=True)
    st.markdown(f'<div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;"><div class="card-ico">📈</div><div><strong style="font-size:17px;color:white !important;">Time Series{suffix}</strong><p style="margin:2px 0 0 0;font-size:12px;color:rgba(255,255,255,0.85) !important;">Completed transfer trends · Month: 10th → 10th</p></div></div>', unsafe_allow_html=True)
    dd = k.get("done_df", pd.DataFrame())
    fd = k.get("failed_df", pd.DataFrame())
    lo = dict(height=380, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)',
              font=dict(family='Inter', color='white'), margin=dict(t=40, b=40, l=40, r=20),
...
# ============================================================
# ADMIN LOGIN CTA (bottom of dashboard)
# ============================================================
def view_admin_cta():
    st.markdown(f"""
    <div class="admin-cta">
        <div class="admin-cta-info">
            <div class="admin-cta-ico">🛡️</div>
            <div>
                <strong style="font-size:17px;color:white !important;">Admin Access</strong>
                <p style="margin:3px 0 0 0;font-size:12px;color:rgba(255,255,255,0.6) !important;">
                    View 10x sales counts, failed transfers per agent, and detailed management tools
                </p>
            </div>
        </div>
    </div>""", unsafe_allow_html=True)
    ...
    with col2:
        if st.button("🔐 Admin Login", use_container_width=True, key="admin_login_bottom"):
...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.is_admin:
            ...
            <div style="background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.2);
                ...
                <span style="color:rgba(255,255,255,0.6) !important;font-size:11px;">10x sales · Failed transfers visible</span>
            </div>""", unsafe_allow_html=True)
    ...
    if is_admin:
        r = ranges()
        ms = r["month"][0].strftime("%b %d")
        me = r["month"][1].strftime("%b %d")
        st.markdown(f"""
        ...
        <div class="success-banner">
            ✅ Loaded <strong>{len(df)}</strong> records — <strong>{k['done']} completed</strong> transfers · {k['pend']} pending · Month: {ms} → {me}
        </div>""", unsafe_allow_html=True)
    ...
    # Admin login CTA at the bottom (only for standard view)
    if not is_admin:
        view_admin_cta()
